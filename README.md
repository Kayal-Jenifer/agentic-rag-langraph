...
→ Stored permanently → Split → Embedded → Saved into FAISS.
2.User asks a question
→ Sent to backend → ReAct agent evaluates.
3.Agent decides to use search tool
→ Searches FAISS → Retrieves relevant context.
4.Agent composes the final answer
→ Always grounded in retrieved document chunks.
